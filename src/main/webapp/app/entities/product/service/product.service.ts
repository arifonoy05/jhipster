...
type RestOf<T extends IProduct | NewProduct> = Omit<T, 'publishDate'> & {
...
export class ProductService {
  ...
  protected convertDateFromClient<T extends IProduct | NewProduct | PartialUpdateProduct>(product: T): RestOf<T> {
    return {
      ...product,
      publishDate: product.publishDate?.format(DATE_FORMAT) ?? null,
    };
  }
  ...
  protected convertDateFromServer(restProduct: RestProduct): IProduct {
    return {
      ...restProduct,
      publishDate: restProduct.publishDate ? dayjs(restProduct.publishDate) : undefined,
    };
  }
